...
public interface AttendRepository extends JpaRepository<Attend, Attend.AttendId> {
     ...
     */
    @Query(value = "select count(*) * ev.commitmentAmount from Attend aes join Event ev on ev.id = aes.id where aes.participant = :participant group by aes.id, ev.commitmentAmount")
